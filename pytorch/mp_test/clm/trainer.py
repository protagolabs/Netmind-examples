...
def train(dataloader, model, optimizer, args, device):
    ...
    for epoch in range(epoch, args.num_train_epochs):
        ...
        print("Total Steps per epoch: {}".format(len(dataloader)))
        # setup loop
        for batch in dataloader:
            ...
            if t_total % args.log_steps == 0:
                print (f"Step: {t_total} - monitor_metrics: {monitor_metrics}")
            ...
            if t_total % args.save_steps == 0 and args.local_rank == 0:
                ...
                print('Step: {}\tLearning rate: {}\tLoss: {}\t'.format(t_total, scheduler.get_last_lr()[0],
                                                                       np.mean(_loss)))
